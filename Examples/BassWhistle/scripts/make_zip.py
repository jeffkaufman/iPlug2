...
def  main():
  ...
  installer = "\installer\BassWhistle Installer.exe"
   
  if demo:
    installer = "\installer\BassWhistle Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\BassWhistle manual.pdf" 
  ]
  ...
  ZIPNAME = "BassWhistle-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "BassWhistle-v" + FULLVERSIONSTR + "-win-demo.zip"
